...
{$REGION 'Lista de id'}
  IListaId = interface
  ['{C74DA7BB-2B66-449D-A82A-25B8E5ED17F4}']
    function Add(pCampo: String; pIdOld: Integer; pIdNew: Integer): Integer;
    function GetIdOld(pCampo: String): Integer;
    function GetIdNew(pCampo: String; pIdOld: Integer): Integer;
    procedure Clear();
  end;

  TListaId = class(TInterfacedObject, IListaId)
  private type
    TId = record
      Campo: String;
      IdOld: Integer;
      IdNew: Integer;
      Constructor Create(pCampo: String; pIdOld: Integer; pIdNew: Integer);
    end;
  private
    FLista: TList<TId>;
  public
...
{$REGION 'Dataset lists'}
  ...
  public
    ...
    class function ItemListToJSONObject(const AList: TItemList; const AJSONObject: TJSONObject)
      : Boolean; static;
    class function DataSetsToJSONObject(const ADataSets: TFDJSONDataSetsBase; const AJSONObject: TJSONObject)
      : Boolean; static;
    class function JSONObjectToItemList(const AJSONObject: TJSONObject; const AList: TItemList)
      : Boolean; static;
    class function JSONObjectToDataSets(const AJSONObject: TJSONObject; const ADataSets: TFDJSONDataSetsBase)
      : Boolean; static;
    destructor Destroy; override;
  ...
  private
    ...
    [JSONOwned(false)]
    // Prevent unmarshal from freeing FList after construction
    FDataSets: TItemList;
...
{$REGION 'Reader and Writer interfaces'}
  ...
  IFDJSONDeltasApplyUpdates = interface(IFDJSONDeltasReader)
    ...
    function GetonUpdateRecord(): TFDUpdateRecordEvent;
    procedure SetonUpdateRecord(const Value: TFDUpdateRecordEvent);
    property Errors: TFDJSONErrors read GetErrors;
    property onUpdateRecord: TFDUpdateRecordEvent read GetonUpdateRecord write SetonUpdateRecord;
  end;
  ...
  IFDJSONDeltasApplyUpdatesMax = interface(IFDJSONDeltasReader)
    ['{58213D3C-BFE8-4BE3-8197-4236FFC215C8}']
    function ApplyUpdates(const AKey: string; const ASelectCommand: TFDCustomCommand;
      AMaxErrors: Integer = -1): Integer; overload;
    function ApplyUpdates(const Index: Integer; const ASelectCommand: TFDCustomCommand;
      AMaxErrors: Integer = -1): Integer; overload;
    function ApplyUpdates(const AKey: string; const AAdapter: TFDTableAdapter; AMaxErrors: Integer = -1)
      : Integer; overload;
    function ApplyUpdates(const Index: Integer; const AAdapter: TFDTableAdapter; AMaxErrors: Integer = -1)
      : Integer; overload;
    function GetErrors: TFDJSONErrors;
    property Errors: TFDJSONErrors read GetErrors;
  end;
...
{$REGION 'Reader and Writer base classes'}
  ...
  protected
    ...
    class procedure ListAddT<T: TFDAdaptedDataSet>(const ADataList: TFDJSONDataSetsBase; const AName: string;
      const ADataSet: T); overload;
    class procedure ListAddT<T: TFDAdaptedDataSet>(const ADataList: TFDJSONDataSetsBase;
      const ADataSet: T); overload;
  end;
  ...
  protected
    ...
    class function GetListValueByNameT<T: TFDAdaptedDataSet>(const ADataList: TFDJSONDataSetsBase;
      const AName: string): T;
    class function GetListKeyT<T: TFDAdaptedDataSet>(const ADataList: TFDJSONDataSetsBase;
      I: Integer): string;
    class function GetListItemT<T: TFDAdaptedDataSet>(const ADataList: TFDJSONDataSetsBase; I: Integer)
      : TPair<string, T>;
    class function ExtractListItemT<T: TFDAdaptedDataSet>(const ADataList: TFDJSONDataSetsBase; I: Integer)
      : TPair<string, T>;
  end;
{$ENDREGION}
{$REGION 'Reader and Writer classes'}
  ...
  public
    ...
    class function GetListValueByName(const ADataList: TFDJSONDataSets; const AName: string)
  ...
  public
    { public class methods }
    class procedure ListAdd(const ADeltaList: TFDJSONDeltas; const AName: string;
      const AFDMemTable: TFDMemTable); overload; static;
    class procedure ListAdd(const ADeltaList: TFDJSONDeltas; const AFDMemTable: TFDMemTable);
      overload; static;
  public
  ...
  public
    ...
    class function GetListValueByName(const ADataList: TFDJSONDeltas; const AName: string)
      : TFDMemTable; static;
  ...
  private
    ...
    class var FListaID: IListaId;
  protected
    class function ApplyUpdates(const ADelta: TFDMemTable; const AAdapter: TFDTableAdapter;
      const AErrors: TFDJSONErrors): Integer; overload;
    class function ApplyUpdates(const ADelta: TFDMemTable; const ACommand: TFDCustomCommand;
      const AErrors: TFDJSONErrors): Integer; overload;
  ...
  public
    { public class methods }
    class function ListApplyUpdates(const ADeltaList: TFDJSONDeltas; const AKey: string;
      const ASelectCommand: TFDCustomCommand; const AErrors: TFDJSONErrors = nil): Integer; overload; static;
    class function ListApplyUpdates(const ADeltaList: TFDJSONDeltas; const Index: Integer;
      const ASelectCommand: TFDCustomCommand; const AErrors: TFDJSONErrors = nil): Integer; overload; static;
    class function ListApplyUpdates(const ADeltaList: TFDJSONDeltas; const AKey: string;
      const AAdapter: TFDTableAdapter; const AErrors: TFDJSONErrors = nil): Integer; overload; static;
    class function ListApplyUpdates(const ADeltaList: TFDJSONDeltas; const Index: Integer;
      const AAdapter: TFDTableAdapter; const AErrors: TFDJSONErrors = nil): Integer; overload; static;
    class function ListaID(): IListaID;
  public
  ...
  protected
    class function ApplyUpdates(const ADelta: TFDMemTable; const AAdapter: TFDTableAdapter;
      AMaxErrors: Integer; const AErrors: TFDJSONErrors): Integer; overload;
    class function ApplyUpdates(const ADelta: TFDMemTable; const ACommand: TFDCustomCommand;
      AMaxErrors: Integer; const AErrors: TFDJSONErrors): Integer; overload;
  ...
  public
    { public class methods }
    class function ListApplyUpdates(const ADeltaList: TFDJSONDeltas; const AKey: string;
      const ASelectCommand: TFDCustomCommand; AMaxErrors: Integer = -1; const AErrors: TFDJSONErrors = nil)
      : Integer; overload; static;
    class function ListApplyUpdates(const ADeltaList: TFDJSONDeltas; const Index: Integer;
      const ASelectCommand: TFDCustomCommand; AMaxErrors: Integer = -1; const AErrors: TFDJSONErrors = nil)
      : Integer; overload; static;
    class function ListApplyUpdates(const ADeltaList: TFDJSONDeltas; const AKey: string;
      const AAdapter: TFDTableAdapter; AMaxErrors: Integer = -1; const AErrors: TFDJSONErrors = nil): Integer;
      overload; static;
    class function ListApplyUpdates(const ADeltaList: TFDJSONDeltas; const Index: Integer;
      const AAdapter: TFDTableAdapter; AMaxErrors: Integer = -1; const AErrors: TFDJSONErrors = nil): Integer;
      overload; static;
...
{$REGION 'Error types'}
  ...
  public
    ...
    procedure UpdateError(ASender: TDataSet; AException: EFDException; ARow: TFDDatSRow;
...
uses
  ...
  System.NetEncoding, FireDAC.DApt.Intf, System.ZLib, ServerMethodsUnit1;
...
class function TFDJSONInterceptor.DataSetsToJSONObject(const ADataSets: TFDJSONDataSetsBase;
...
begin
  ...
  try
    ...
    Result.MasterFields := AFDMemTable.MasterFields;
    Result.DetailFields := AFDMemTable.DetailFields;
  finally
    AFDMemTable.ResourceOptions.StoreItems := LStoreItems;
  end;
end;

class function TFDJSONInterceptor.ItemListToJSONObject(const AList: TItemList;
  const AJSONObject: TJSONObject): Boolean;
var
...
class function TFDJSONInterceptor.JSONObjectToDataSets(const AJSONObject: TJSONObject;
  const ADataSets: TFDJSONDataSetsBase): Boolean;
var
  LList: TFDJSONDataSetsBase.TItemList;
begin
  LList := TFDJSONDataSetsBase.TItemList.Create;
  Result := JSONObjectToItemList(AJSONObject, LList);
  Assert(ADataSets.FDataSets = nil);
  ADataSets.FDataSets := LList;
  ADataSets.FOwnsDataSets := true;
end;

class function TFDJSONInterceptor.JSONObjectToItemList(const AJSONObject: TJSONObject;
  const AList: TItemList): Boolean;
var
  LMemTable: TFDMemTable;
  LJSONPair: TJSONPair;
...
procedure TFDJSONErrors.UpdateError(ASender: TDataSet; AException: EFDException; ARow: TFDDatSRow;
...
procedure TFDJSONDataSetsWriter.Add(const AName: string; const ADataSet: TFDAdaptedDataSet);
begin
  ListAdd(FDataSetList, AName, ADataSet);
end;

procedure TFDJSONDataSetsWriter.Add(const ADataSet: TFDAdaptedDataSet);
begin
...
class procedure TFDJSONDataSetsWriter.ListAdd(const ADataList: TFDJSONDataSets; const AName: string;
...
class function TFDJSONDataSetsReader.ExtractListItem(const ADataList: TFDJSONDataSets; I: Integer): TItemPair;
begin
...
class function TFDJSONDataSetsReader.GetListCount(const ADataList: TFDJSONDataSets): Integer;
begin
  Result := GetListCountT(ADataList);
end;

class function TFDJSONDataSetsReader.GetListItem(const ADataList: TFDJSONDataSets; I: Integer): TItemPair;
begin
  Result := GetListItemT<TFDAdaptedDataSet>(ADataList, I);
end;

class function TFDJSONDataSetsReader.GetListKey(const ADataList: TFDJSONDataSets; I: Integer): string;
begin
  Result := GetListKeyT<TFDAdaptedDataSet>(ADataList, I);
end;

class function TFDJSONDataSetsReader.GetListReader(const ADataList: TFDJSONDataSets): IFDJSONDataSetsReader;
begin
  Result := TFDJSONDataSetsReader.Create(ADataList);
end;

class function TFDJSONDataSetsReader.GetListValue(const ADataList: TFDJSONDataSets; I: Integer)
  : TFDAdaptedDataSet;
begin
  Result := GetListValueT<TFDAdaptedDataSet>(ADataList, I);
end;

class function TFDJSONDataSetsReader.GetListValueByName(const ADataList: TFDJSONDataSets; const AName: string)
  : TFDAdaptedDataSet;
begin
...
function TFDJSONDataSetsReader.GetValueByName(const AName: string): TFDAdaptedDataSet;
begin
...
class function TFDJSONDeltasReader.ExtractListItem(const ADataList: TFDJSONDeltas; I: Integer): TItemPair;
begin
...
class function TFDJSONDeltasReader.GetListCount(const ADataList: TFDJSONDeltas): Integer;
begin
  Result := GetListCountT(ADataList);
end;

class function TFDJSONDeltasReader.GetListItem(const ADataList: TFDJSONDeltas; I: Integer): TItemPair;
begin
  Result := GetListItemT<TFDMemTable>(ADataList, I);
end;

class function TFDJSONDeltasReader.GetListKey(const ADataList: TFDJSONDeltas; I: Integer): string;
begin
  Result := GetListKeyT<TFDMemTable>(ADataList, I);
end;

class function TFDJSONDeltasReader.GetListValue(const ADataList: TFDJSONDeltas; I: Integer): TFDMemTable;
begin
  Result := GetListValueT<TFDMemTable>(ADataList, I);
end;

class function TFDJSONDeltasReader.GetListValueByName(const ADataList: TFDJSONDeltas; const AName: string)
  : TFDMemTable;
begin
...
class function TFDJSONDataSetsReaderBase.ExtractListItemT<T>(const ADataList: TFDJSONDataSetsBase; I: Integer)
  : TPair<string, T>;
begin
...
class function TFDJSONDataSetsReaderBase.GetListCountT(const ADataList: TFDJSONDataSetsBase): Integer;
begin
  if GetPairList(ADataList) <> nil then
...
class function TFDJSONDataSetsReaderBase.GetListItemT<T>(const ADataList: TFDJSONDataSetsBase; I: Integer)
  : TPair<string, T>;
begin
  if GetPairList(ADataList) <> nil then
    Result := TPair<string, T>.Create(GetPairList(ADataList)[I].Key, T(GetPairList(ADataList)[I].Value))
...
class function TFDJSONDataSetsReaderBase.GetPairList(const AList: TFDJSONDataSetsBase): TPairList;
begin
...
class function TFDJSONDataSetsReaderBase.GetListValueByNameT<T>(const ADataList: TFDJSONDataSetsBase;
  const AName: string): T;
var
  I: Integer;
begin
...
function InternalApplyUpdates(const ADelta: TFDMemTable; const AAdapter: TFDTableAdapter; AMaxErrors: Integer;
...
begin
  ...
  try
    ...
    LFDMemTable.Name := ADelta.Name;
    if Assigned(ADelta.OnUpdateRecord) then
    begin
      LFDMemTable.OnUpdateRecord := ADelta.OnUpdateRecord;
    end;
...
class function TFDJSONDeltasApplyUpdates.ApplyUpdates(const ADelta: TFDMemTable;
...
class function TFDJSONDeltasApplyUpdates.ApplyUpdates(const ADelta: TFDMemTable;
  const ACommand: TFDCustomCommand; const AErrors: TFDJSONErrors): Integer;
var
  LFDAdapter: TFDTableAdapter;
begin
  LFDAdapter := nil;
...
function TFDJSONDeltasApplyUpdates.ApplyUpdates(const AKey: string;
  const ASelectCommand: TFDCustomCommand): Integer;
var
  LMemTable: TFDMemTable;
begin
  ...
  begin
    LMemTable.onUpdateRecord := FonUpdateRecord;
  end;
  Result := ApplyUpdates(LMemTable, ASelectCommand, FErrors);
end;

function TFDJSONDeltasApplyUpdates.ApplyUpdates(const Index: Integer;
  const ASelectCommand: TFDCustomCommand): Integer;
var
  LMemTable: TFDMemTable;
begin
...
function TFDJSONDeltasApplyUpdates.ApplyUpdates(const Index: Integer;
  const AAdapter: TFDTableAdapter): Integer;
var
  LDelta: TFDMemTable;
begin
...
function TFDJSONDeltasApplyUpdates.GetonUpdateRecord: TFDUpdateRecordEvent;
begin
  result := FonUpdateRecord;
end;

class function TFDJSONDeltasApplyUpdates.ListApplyUpdates(const ADeltaList: TFDJSONDeltas;
  const Index: Integer; const ASelectCommand: TFDCustomCommand; const AErrors: TFDJSONErrors): Integer;
var
  LDelta: TFDMemTable;
begin
...
class function TFDJSONDeltasApplyUpdates.ListApplyUpdates(const ADeltaList: TFDJSONDeltas; const AKey: string;
  const ASelectCommand: TFDCustomCommand; const AErrors: TFDJSONErrors): Integer;
var
  LDelta: TFDMemTable;
begin
...
class function TFDJSONDeltasApplyUpdates.ListaID: IListaID;
begin
  result := FListaID;
end;

class function TFDJSONDeltasApplyUpdates.ListApplyUpdates(const ADeltaList: TFDJSONDeltas;
  const Index: Integer; const AAdapter: TFDTableAdapter; const AErrors: TFDJSONErrors): Integer;
var
  LDelta: TFDMemTable;
begin
  LDelta := GetListValue(ADeltaList, Index);
...
procedure TFDJSONDeltasApplyUpdates.SetonUpdateRecord(const Value: TFDUpdateRecordEvent);
begin
...
class function TFDJSONDeltasApplyUpdatesMax.ApplyUpdates(const ADelta: TFDMemTable;
  const AAdapter: TFDTableAdapter; AMaxErrors: Integer; const AErrors: TFDJSONErrors): Integer;
begin
  Result := InternalApplyUpdates(ADelta, AAdapter, AMaxErrors, AErrors);
end;

class function TFDJSONDeltasApplyUpdatesMax.ApplyUpdates(const ADelta: TFDMemTable;
  const ACommand: TFDCustomCommand; AMaxErrors: Integer; const AErrors: TFDJSONErrors): Integer;
var
  LFDAdapter: TFDTableAdapter;
begin
  LFDAdapter := nil;
...
function TFDJSONDeltasApplyUpdatesMax.ApplyUpdates(const Index: Integer;
  const ASelectCommand: TFDCustomCommand; AMaxErrors: Integer): Integer;
var
  LMemTable: TFDMemTable;
begin
  LMemTable := GetValue(Index);
...
function TFDJSONDeltasApplyUpdatesMax.ApplyUpdates(const Index: Integer; const AAdapter: TFDTableAdapter;
  AMaxErrors: Integer): Integer;
var
  LDelta: TFDMemTable;
begin
...
class function TFDJSONDeltasApplyUpdatesMax.ListApplyUpdates(const ADeltaList: TFDJSONDeltas;
  const Index: Integer; const ASelectCommand: TFDCustomCommand; AMaxErrors: Integer;
  const AErrors: TFDJSONErrors): Integer;
var
  LDelta: TFDMemTable;
begin
...
class function TFDJSONDeltasApplyUpdatesMax.ListApplyUpdates(const ADeltaList: TFDJSONDeltas;
  const AKey: string; const ASelectCommand: TFDCustomCommand; AMaxErrors: Integer;
  const AErrors: TFDJSONErrors): Integer;
var
  LDelta: TFDMemTable;
begin
...
class function TFDJSONDeltasApplyUpdatesMax.ListApplyUpdates(const ADeltaList: TFDJSONDeltas;
  const Index: Integer; const AAdapter: TFDTableAdapter; AMaxErrors: Integer;
  const AErrors: TFDJSONErrors): Integer;
var
  LDelta: TFDMemTable;
begin
...
class function TFDJSONDeltasApplyUpdatesMax.ListApplyUpdates(const ADeltaList: TFDJSONDeltas;
  const AKey: string; const AAdapter: TFDTableAdapter; AMaxErrors: Integer;
  const AErrors: TFDJSONErrors): Integer;
var
  LDelta: TFDMemTable;
begin
...
class function TFDJSONDataSetsWriterBase.GetPairList(const AList: TFDJSONDataSetsBase): TPairList;
begin
  if (AList = nil) or (AList.FDataSets = nil) then
    raise EFDDataListError.Create(sCantReadNilList);
  Result := AList.FDataSets;
...
class procedure TFDJSONDataSetsWriterBase.ListAddT<T>(const ADataList: TFDJSONDataSetsBase;
  const ADataSet: T);
begin
...
class procedure TFDJSONDataSetsWriterBase.ListAddT<T>(const ADataList: TFDJSONDataSetsBase;
  const AName: string; const ADataSet: T);
begin
  CreateList(ADataList);
  GetPairList(ADataList).Add(TPair<string, TFDAdaptedDataSet>.Create(AName, ADataSet));
end;

function TListaId.Add(pCampo: String; pIdOld, pIdNew: Integer): Integer;
begin
  Result := FLista.Add(TId.Create(pCampo, pIdOld, pIdNew));
end;

procedure TListaId.Clear;
begin
  FLista.Clear();
end;

constructor TListaId.Create;
begin
  FLista := TList<TId>.Create();
end;

destructor TListaId.Destroy;
begin
  FLista.Clear();
  FreeAndNil(FLista);
  inherited;
...
function TListaId.GetIdNew(pCampo: String; pIdOld: Integer): Integer;
var
  Item: TId;
begin
  Result := 0;
  ...
  begin
    if (Item.Campo = pCampo) and (Item.IdOld = pIdOld) then
    begin
      Result := Item.IdNew;
      Break;
    end;
...
function TListaId.GetIdOld(pCampo: String): Integer;
var
  Item: TId;
begin
  Result := 0;
...
constructor TListaId.TId.Create(pCampo: String; pIdOld, pIdNew: Integer);
begin
  Campo := pCampo;
  IdOld := pIdOld;
  IdNew := pIdNew;
