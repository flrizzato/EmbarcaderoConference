...
type
  ...
  public
    ...
    function DoGetUserDevices(const UserId: string): TJSONArray;
    //Registro
    function DoUpdateDevice(DeviceInfo: TPushDevice): integer;
    function DoUpdatePushOption(const DeviceId: string; PushOption: boolean): integer;
    //Envio
...
function TdmPush.DoGetUserDevices(const UserId: string): TJSONArray;
begin
  Result:= TJSONArray.Create;
  try
    qryUserDevices.ParamByName('User_Id').AsString:= UserId;
    qryUserDevices.Open;
    if not qryUserDevices.IsEmpty then
      result:= qryUserDevices.AllRowsAsJSONArray;
  finally
    qryUserDevices.Close;
  end;
end;

function TdmPush.DoUpdateDevice(DeviceInfo: TPushDevice): integer;
begin
  try
    qryUpdateDevice.ParamByName('DEVICE_ID').AsString:= DeviceInfo.DeviceId;
    qryUpdateDevice.ParamByName('USER_ID').AsString:= DeviceInfo.UserId;
    qryUpdateDevice.Open;
    if qryUpdateDevice.IsEmpty then
    begin
      qryUpdateDevice.Insert;
      qryUpdateDevice.FieldByName('USER_ID').AsString:= DeviceInfo.UserId;
    ...
    qryUpdateDevice.FieldByName('USER_NAME').AsString:= DeviceInfo.UserName;
...
function TdmPush.DoUpdatePushOption(const DeviceId: string; PushOption: boolean): integer;
const
  PushValues: array[boolean] of string = ('N', 'S');
begin
  result:= 0;
  try
    qryUpdatePush.ParamByName('DEVICE_ID').AsString:= DeviceId;
    qryUpdatePush.Open;
    if not qryUpdatePush.IsEmpty then
    begin
      qryUpdatePush.Edit;
      qryUpdatePush.FieldByName('DT_UPDATE').AsDateTime:= Now;
      qryUpdatePush.FieldByName('RECEIVE_PUSH').AsString:= PushValues[PushOption];
      qryUpdatePush.Post;
      result:= qryUpdatePush.ChangeCount;
      qryUpdatePush.ApplyUpdates(0);
    end;
  finally
    qryUpdatePush.Close;
  end;
...
var
  AppleTokens, GoogleTokens: TStrings;
  Total: integer;
begin
  Result:= TJSONObject.Create;
  AppleTokens:= TStringList.Create;
  GoogleTokens:= TStringList.Create;
  try
    qrySendToUser.ParamByName('user_id').AsString:= UserId;
    qrySendToUser.Open;
    while not qrySendToUser.EOF do
    begin
      if qrySendToUser.FieldByName('os_name').AsString = ANDROID_OS then
        GoogleTokens.Add(qrySendToUser.FieldByName('device_token').AsString)
      else
      if qrySendToUser.FieldByName('os_name').AsString = IOS_OS then
        AppleTokens.Add(qrySendToUser.FieldByName('device_token').AsString);
      qrySendToUser.Next;
    end;
    Total:= SendToGoogle(Title, Msg, GoogleTokens) +
            SendToApple(Title, Msg, AppleTokens);
    qrySendToUser.Close;
  finally
    AppleTokens.Free;
    GoogleTokens.Free;
