...
uses
  ...
  Data.DB, DataSnap.DSClientRest, FireDAC.Comp.DataSet, FireDAC.Comp.Client,
  System.Rtti,
...
type
  TDepartmentsClientForm = class(TForm)
    ...
    Panel1: TPanel;
    butFileDownload: TButton;
    butFileUpload: TButton;
    ...
    procedure butFileDownloadClick(Sender: TObject);
    procedure butFileUploadClick(Sender: TObject);
  private
...
procedure HandleRESTException(const AConnection: TDSRestConnection;
  const APrefix: string; const E: TDSRestProtocolException);
var
...
begin
  try
    ...
    LDataSetList := ClientModule2.ServerMethods1Client.
      GetDepartmentEmployees(ADEPTNO);
    // Update UI
...
procedure TDepartmentsClientForm.butFileDownloadClick(Sender: TObject);
var
...
begin
  sFileName := sDataDir + 'file-to-download-copy.pdf';
  ...
  begin
    ...
    try
      fStream := ClientModule2.ServerMethods1Client.FileDownload
    ...
    if FileExists(sFileName) then
      ShowMessage('File transfer ok!')
    else
      ShowMessage('File transfer failed!');
  end

  else
    ShowMessage('Cannot remove the previous file!');
end;

procedure TDepartmentsClientForm.butFileUploadClick(Sender: TObject);
var
  fFileStream: TFileStream;
begin
  fFileStream := TFileStream.Create(sDataDir + 'file-to-upload.jpg',
    fmOpenRead);
  ClientModule2.ServerMethods1Client.FileUpload(fFileStream);
end;
