...
uses
  ...
  ToolWin, Grids, DBGrids, StdCtrls, Buttons, ExtCtrls;
...
begin
  ...
  try
    if (cmbField.ItemIndex >= 0) and (edtValue.Text <> '') then
      datControl.DataSet.Locate(aFieldNames[cmbField.ItemIndex], edtValue.Text, [loCaseInsensitive, loPartialKey]);
  finally
...
begin
  ...
  for i := 0 to datControl.DataSet.Fields.Count - 1 do
    if (datControl.DataSet.Fields[i].FieldKind = fkData) and (datControl.DataSet.Fields[i].Tag <> -2) then
    begin
      cmbField.Items.Add(datControl.DataSet.Fields[i].DisplayLabel);
      SetLength(aFieldNames, Length(aFieldNames) + 1);
      aFieldNames[Length(aFieldNames) - 1] := datControl.DataSet.Fields[i].FieldName;
    end;
