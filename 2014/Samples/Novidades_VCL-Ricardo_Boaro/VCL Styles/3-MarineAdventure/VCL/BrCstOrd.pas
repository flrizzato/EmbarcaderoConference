...
uses QryCust, EdCust, EdOrders, DataMod;
...
begin
  MastData.CustMasterSrc.Dataset := MastData.cdsCust;
  MastData.cdsCust.Locate('CustNo', NewCustNo, []);
end;

function TBrCustOrdForm.GetOrderNo: Double;
begin
  Result := MastData.cdsOrdByCustOrderNo.Value;
end;

procedure TBrCustOrdForm.SetOrderNo(NewOrderNo: Double);
begin
  MastData.cdsOrdByCust.Locate('OrderNo', NewOrderNo, []);
end;
...
begin
  if not ActivateBtn.Down then
    ...
    try
      MastData.cdsCustQuery.Close;
      with MastData.qryCustQuery do begin
        Params[0].AsDatetime := QueryCustDlg.FromDate;
        Params[1].AsDatetime := QueryCustDlg.ToDate;
      end;
      with MastData.cdsCustQuery do begin
        Open;
        { Any records in the result set? }
        if BOF and EOF then Abort;
        MastData.CustMasterSrc.Dataset := MastData.cdsCustQuery;
      end;
    except
      MastData.CustMasterSrc.Dataset := MastData.cdsCust;
...
begin
  MastData.cdsCust.Open;
  MastData.cdsCust.First;
end;
